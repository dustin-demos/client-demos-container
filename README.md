...
- **currency** — a currency converter I built as a job-application code challenge. I didn't get the job, but I liked the demo enough to keep it on my site for years as an example. Vue 3 + Vite; built here with `--base=/currency/` so it serves under its path.
- **resize** — an aspect-ratio calculator I actually built for myself: cropping art for my Instagram at odd canvas sizes, then resizing to a square or 8x10, meant doing the math by hand, so I made a tool to do it. Superfine + an esbuild/sass build; the only change here pins `uglify-js` to `3.13.8`, the version it was written against (newer 3.x can't minify it).
