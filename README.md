...
- **reasonable** — a visual node editor for building workflows, started **October 2023** and worked on into 2024. It was a proof of concept and never fully finished. Vue 3 + vite-ssg, built with `--base=/reasonable/`. The original repo has a real backend, but it does nothing the demo needs, so only the client is served here. The full history lives in `reasonable-backup`. Client-side routes handled by an SPA fallback in `main.ts`.
  - Set router base to `import.meta.env.BASE_URL` so client-side routes resolve under the subpath.
  - Added the workflow run simulation — the original only set nodes to `pending` and never resolved them; the sequential `pending` → `success` animation (600ms per node) is a revival addition.
  - Hid the raw debug data panels behind a "Show Data / Hide Data" toggle; they were always visible in the original.

- **clickmart** — a mock storefront UI from **July 2021**, with a sidebar of departments, a product market, and a carousel. Built on pocket + Superfine with an esbuild/sass build, pulling pocket and superstatic as git deps (pointed at their `dustin-archive` homes). Served with an SPA fallback in `main.ts`.
  - Prefixed all routes, font paths, favicon, and image asset URLs with `/clickmart/`.
  - Fixed a `SourceCodProe` → `SourceCodePro` font folder typo.
  - Pinned pocket to commit `#394d4fe7d` to lock a stable revision.
...
- **onclick notes** — a serverless markdown notes app from **April 2021** that packs the entire note into the URL (raw DEFLATE plus base64url), so there's nothing to store. Built on pocket + Superfine, pulling pocket and superstatic as git deps (pointed at their `dustin-archive` homes). Served with an SPA fallback in `main.ts`.
  - Prefixed all routes, font preloads, favicon, and logo asset paths with `/onclick-notes/`.
  - Fixed a `SourceCodProe` → `SourceCodePro` font folder typo.
  - Pinned pocket to `#394d4fe7d` and pinned `hast-to-hyperscript`, `mdast-util-from-markdown`, `mdast-util-to-hast`, and `superfine` to exact versions.
  - Switched note URL encoding from standard base64 to base64url (RFC 4648, URL-safe alphabet, no padding) so note links survive query strings without escaping.

- **valorant blog editor** — a flat-file, JSON-based CMS from **March 2021** for a friend's Valorant tips blog. I messed around with it mostly for fun, ended up spending all my time on the drag-and-drop editor, and never finished, so they never got the blog. Built on pocket + Superfine with an esbuild/sass build. Served with an SPA fallback in `main.ts`.
  - Prefixed all routes and the editor back-link with `/valorant-blog/`.
  - Pinned `sass` to `1.32.8` so the SVG icon fills render as hex instead of being converted to `rgb()` by newer sass.
  - Prefixed the Anton font URL in `_fonts.scss` with `/valorant-blog/`.

- **state sync** — a proof of concept from **February 2021** for syncing state across differently-sized frames over a `BroadcastChannel`. The same page loads in three differently-sized iframes; type or scroll in one and the others follow, synced locally with no server. Built on pocket + Superfine. Client-side routes handled by an SPA fallback in `main.ts`.
  - Prefixed all page routes with `/state-sync/`.
  - Replaced the hardcoded dead Netlify iframe URL with the same-origin `/state-sync/demo`.
  - Updated the route check in `sync.js` from `'/demo'` to `'/state-sync/demo'` so the BroadcastChannel sync actually initializes under the new path.

- **resize** — an aspect-ratio calculator I built for myself in **February 2021**: cropping art for my Instagram at odd canvas sizes, then resizing to a square or 8x10, meant doing the math by hand, so I made a tool to do it. Superfine + an esbuild/sass build.
  - Pinned `uglify-js` to `3.13.8` (newer 3.x can't minify it).
  - Pinned `sass` to `1.34.1` (newer sass renders `lab()` colors as `rgb()`, which broke the SVG icon fills that expect a hex).
  - Set the home route to `/resize/` so it matches the path it's served under.
  - Replaced dead `via.placeholder.com` placeholder image URLs with `placehold.co`.

- **discord message queue** — a tool from **December 2020** that queues chat messages and drips them into a Discord channel on a timer, to optimize a points bot in a community I was in. You supply your own token and channel id. Built on pocket + Superfine. Served with an SPA fallback in `main.ts`.
  - Prefixed all routes, font, favicon, and manifest paths with `/discord-queue/`.
...
- **codepen demos** — a collection of small experiments originally published on CodePen. Each was written in Pug + Sass + vanilla JS (or Vue 2 / jQuery), using the tooling CodePen provided at the time. They live here under `demos/codepen/` and are compiled at build time with the pug and sass Node APIs.
  - **Framework Challenge** (Apr 2021) — the same UI built in three frameworks: Pocket JS, React, and Vue 3. No changes to the source.
  ...
  - **CSS 3D Toggle Button** (Sep 2016) — a toggle button with a CSS 3D flip animation. No changes to the source.
  - **Vue Pagination** (Sep 2016) — a pagination component built in Vue 2.0.0-rc.7.
    - Commented out the static fallback item list (a non-Vue duplicate rendered below the animated one) so only the animated list shows.
